...
export default function MitraQrDisplay() {
  ...
  const [dynamicToken, setDynamicToken] =
    useState('TOKEN-1234');
  ...
  useEffect(() => {
    const fetchMitraTrips = async () => {
      ...
      try {
        ...
        console.error(
          'Gagal memuat trip aktif:',
          err
        );

        toast.error(
          'Gagal menyambungkan data QR ke server.',
          {
            title: 'Error'
          }
        );
  ...
  useEffect(() => {
    ...
    const qrPayload =
      `TRIP-${current.id}|Route:${current.originPoint?.name}->${current.destinationPoint?.name}|TS:${dynamicToken}`;
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] sm:text-[11px] text-neutral-400 mt-0.5">
            Tunjukkan QR Code dinamis ini kepada
            Operator Pos saat verifikasi
            keberangkatan atau kedatangan di pos
            mitra.
          </p>
        </div>
      </div>
