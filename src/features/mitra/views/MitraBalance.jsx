...
const STATUS_LABEL = {
  Aktif: 'Menunggu Keberangkatan',
  'In Transit': 'Dalam Perjalanan (Escrow Hold)',
};
...
export default function MitraBalance() {
  ...
  const escrowTransactions = trips.filter(
    (t) => t.status === 'Aktif' || t.status === 'In Transit'
  );
  ...
  const handleWithdraw = (e) => {
    ...
    if (!amount || amount < MIN_WITHDRAWAL) {
      toast.warning(
        `Minimum penarikan saldo adalah Rp ${MIN_WITHDRAWAL.toLocaleString(
          'id-ID'
        )}.`,
    ...
    if (!result.ok) {
      toast.warning(
        'Penarikan gagal diproses. Silakan coba lagi.',
        { title: 'Gagal' }
      );
      return;
    }
  ...
  const handleSaveBank = (e) => {
    ...
    if (
      ...
      toast.warning(
        'Semua field rekening bank wajib diisi.',
                ...
                <tbody className="divide-y divide-neutral-100 text-[9px]">
                  {escrowTransactions.length === 0 ? (
                    <tr>
                      <td
                        colSpan={4}
                        className="py-6 text-center text-neutral-400 font-medium"
                      >
                    ...
                    escrowTransactions.map((tx) => (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-3 font-bold text-neutral-800">
                          Rp {tx.escrowAmount.toLocaleString('id-ID')}
                        </td>
      ...
      >
        <div className="text-center space-y-3 text-[10px]">
          ...
          <p className="text-neutral-500">
            Dana Anda sedang diproses oleh Payment Gateway dan akan
            masuk ke rekening dalam 1x24 jam. Status akan berubah
            menjadi "Berhasil" setelah bank mengonfirmasi.
          </p>
