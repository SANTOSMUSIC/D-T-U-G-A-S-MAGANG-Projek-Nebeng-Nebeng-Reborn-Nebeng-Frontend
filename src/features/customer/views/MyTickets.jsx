    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 min-h-screen font-['Inter']">
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          className="w-full pl-9 pr-3.5 py-2.5 bg-white border border-neutral-200 rounded-xl text-[10px] font-medium text-neutral-800 focus:outline-none shadow-sm"
        />
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-3.5 py-1.5 rounded-xl text-[10px] font-bold transition cursor-pointer ${
            activeTab === 'aktif' ? 'text-white shadow-sm' : 'bg-white text-neutral-600 border border-neutral-200 hover:bg-neutral-50'
          }`}
          style={activeTab === 'aktif' ? { backgroundColor: PRIMARY_COLOR } : undefined}
        >
        ...
        <button
          ...
          className={`px-3.5 py-1.5 rounded-xl text-[10px] font-bold transition cursor-pointer ${
            activeTab === 'riwayat' ? 'text-white shadow-sm' : 'bg-white text-neutral-600 border border-neutral-200 hover:bg-neutral-50'
          }`}
        ...
        <button
          ...
          className={`px-3.5 py-1.5 rounded-xl text-[10px] font-bold transition cursor-pointer flex items-center gap-1.5 ${
            activeTab === 'reward' ? 'text-white shadow-sm' : 'bg-white text-neutral-600 border border-neutral-200 hover:bg-neutral-50'
          }`}
          style={activeTab === 'reward' ? { backgroundColor: PRIMARY_COLOR } : undefined}
        >
            ...
            filteredTickets.map((ticket) => (
              <div key={ticket.id} className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 flex flex-col justify-between space-y-4">
                <div className="space-y-3">
                  ...
                  {ticket.otp && (
                    <div className="p-2.5 rounded-xl flex items-center justify-between text-[9px] border" style={{ backgroundColor: `${PRIMARY_ACCENT}22`, borderColor: `${PRIMARY_ACCENT}55` }}>
                      <span className="font-bold flex items-center gap-1" style={{ color: PRIMARY_COLOR }}>
                        <KeyRound className="w-3 h-3" /> OTP Penyerahan Barang:
                      </span>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-[11px] tracking-wider" style={{ color: PRIMARY_COLOR }}>
                          {showOtpMap[ticket.id] ? ticket.otp : '******'}
                        </span>
                        <button onClick={() => toggleOtpVisibility(ticket.id)} className="cursor-pointer transition" style={{ color: PRIMARY_COLOR }}>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {ticket.status === 'Aktif' && (
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg text-[9px] font-bold transition flex items-center gap-1 cursor-pointer border"
                          style={{ backgroundColor: `${PRIMARY_ACCENT}22`, color: PRIMARY_COLOR, borderColor: `${PRIMARY_ACCENT}55` }}
                        >
                      ...
                      {ticket.status === 'Aktif' && ticket.currentStatusText === CANCELLABLE_STATUS_TEXT && (
                        <button onClick={() => openModal(ticket, 'cancel')} className="px-2.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 rounded-lg text-[9px] font-bold transition flex items-center gap-1 cursor-pointer">
                      ...
                      {ticket.status === 'Aktif' && (
                        <button onClick={() => openModal(ticket, 'qr')} className="px-2.5 py-1.5 text-white rounded-lg text-[9px] font-bold transition flex items-center gap-1 cursor-pointer shadow-sm" style={{ backgroundColor: PRIMARY_COLOR }}>
        ...
        <div className="space-y-3 text-[10px]">
              ...
              chatMessages.map((msg, index) => {
                ...
                return (
                  <div key={index} className={`flex flex-col max-w-[80%] ${isMe ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                    ...
                    <div className={`p-2.5 rounded-2xl text-[10px] ${isMe ? 'text-white rounded-br-none' : 'bg-white border border-neutral-200 text-neutral-800 rounded-bl-none shadow-sm'}`} style={isMe ? { backgroundColor: PRIMARY_COLOR } : undefined}>
                      {msg.messageText}
                    </div>
                  </div>
                );
          ...
          <form onSubmit={handleSendChatMessage} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl focus:outline-none"
            />
            <button type="submit" disabled={!newMessageText.trim()} className="px-4 py-2.5 text-white rounded-xl font-bold transition shadow-sm cursor-pointer disabled:opacity-50 flex items-center gap-1" style={{ backgroundColor: PRIMARY_COLOR }}>
      ...
      >
        <div className="text-center space-y-3">
          ...
          <button onClick={() => setSelectedTicket(null)} className="w-full py-2.5 text-white rounded-xl text-[10px] font-bold transition cursor-pointer shadow-sm" style={{ backgroundColor: PRIMARY_COLOR }}>
            Tutup
          </button>
        </div>
      </BaseModal>
      ...
      >
        <div className="text-center space-y-4 text-[10px]">
          ...
          <div className="flex gap-2">
            <button onClick={() => setSelectedTicket(null)} disabled={isCancelling} className="flex-1 py-2.5 bg-neutral-100 text-neutral-700 rounded-xl font-bold">Tidak, Kembali</button>
            <button onClick={handleCancelTicket} disabled={isCancelling} className="flex-1 py-2.5 bg-rose-600 text-white rounded-xl font-bold flex items-center justify-center gap-1.5 shadow-sm">
              {isCancelling ? <span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><XCircle className="w-3.5 h-3.5" /><span>Ya, Batalkan</span></>}
            </button>
          </div>
        </div>
      </BaseModal>
