...
export default function MyTickets() {
  ...
  // Gunakan useRef agar conversationId selalu sinkron
  // di dalam setInterval polling
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // 1. Fetch Daftar Pesanan/Tiket dari Backend
      ...
      try {
        ...
        if (isMounted) {
          const mappedTickets = (res.data || []).map((order) => {
            ...
            const origin =
              trip.originPoint?.name || 'Pos Asal';

            const destination =
              trip.destinationPoint?.name || 'Pos Tujuan';
            ...
            return {
              ...
              title: isParcel
                ? `Nebeng Barang (${
                    order.itemOrders?.[0]?.itemCategory ||
                    'Paket'
                  })`
                : 'Nebeng Penumpang',
      ...
      // 2. Fetch Reward & Poin dari Backend
      setIsLoadingRewards(true);

      try {
        const resReward =
          await apiClient.get('/rewards/me');

        if (isMounted) {
          // Ambil data langsung dari struktur respons Axios response.data
    ...
    const fetchMessagesPeriodic = async () => {
      ...
      try {
        const resMsg = await apiClient.get(
          `/chat/conversation/${currentId}/messages`
        );
  ...
  const handleCancelTicket = async () => {
    ...
    try {
      await apiClient.patch(
        `/orders/${selectedTicket.rawId}/cancel`
      );

      toast.success(
        'Pesanan berhasil dibatalkan dan kuota dikembalikan.',
        { title: 'Sukses' }
      );
  ...
  const openModal = async (ticket, type) => {
    ...
    if (type === 'chat') {
      ...
      try {
        const resConv = await apiClient.post(
          '/chat/conversation',
          {
        ...
        toast.error(
          'Gagal membuka ruang chat dengan mitra.',
          { title: 'Error' }
        );
      } finally {
  ...
  const handleSendChatMessage = async (e) => {
    ...
    try {
      const res = await apiClient.post(
        `/chat/conversation/${chatConversationId}/messages`,
        {
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 min-h-screen font-['Inter']">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] sm:text-[11px] text-neutral-400 mt-0.5">
            Tunjukkan QR Code digital di Pos, chat langsung
            dengan Mitra, dan kelola Poin Reward Anda.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          <span>
            Poin & Reward ({rewardSummary.totalPoints}{' '}
            Poin)
          </span>
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {isLoadingTickets ? (
            Array.from({ length: 2 }).map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 space-y-3"
              >
            ...
            filteredTickets.map((ticket) => (
              <div
                key={ticket.id}
                className="bg-white rounded-2xl p-5 shadow-sm border border-neutral-200 flex flex-col justify-between space-y-4"
              >
                <div className="space-y-3">
                  ...
                  {ticket.otp && (
                    <div
                      className="p-2.5 rounded-xl flex items-center justify-between text-[9px] border"
                      style={{
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between pt-2 border-t border-neutral-100 flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {ticket.status === 'Aktif' &&
                        ticket.currentStatusText ===
                          CANCELLABLE_STATUS_TEXT && (
                          <button
                            onClick={() =>
                              openModal(ticket, 'cancel')
                            }
                      ...
                      {ticket.status === 'Aktif' && (
                        <button
                          ...
                          className="px-2.5 py-1.5 text-white rounded-lg text-[9px] font-bold transition flex items-center gap-1 cursor-pointer shadow-sm"
                          style={{
                            backgroundColor:
                              PRIMARY_COLOR,
                          }}
              ...
              <EmptyState
                ...
                description={
                  searchQuery.trim()
                    ...
                    : 'Belum ada data perjalanan pada kategori ini.'
                }
              />
            </div>
          )}
            ...
            {isLoadingRewards ? (
              ...
              <div className="space-y-2">
                {rewardSummary.history.map((item) => (
                  <div
                    key={item.id}
                    className="p-3 bg-neutral-50 border border-neutral-100 rounded-xl flex items-center justify-between text-[10px]"
                  >
                    <div>
                      <p className="font-bold text-neutral-800">
                        {item.description ||
        ...
        <div className="space-y-3 text-[10px]">
          <div className="h-72 overflow-y-auto p-3 bg-neutral-50 border border-neutral-200 rounded-xl space-y-2.5 flex flex-col">
            {isLoadingChat ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-neutral-400 space-y-1">
                ...
                <p>
                  Belum ada percakapan. Mulai chat
                  dengan mitra sekarang.
                </p>
              </div>
            )}
          ...
          >
            ...
            <button
              ...
              className="px-4 py-2.5 text-white rounded-xl font-bold transition shadow-sm cursor-pointer disabled:opacity-50 flex items-center gap-1"
              style={{
                backgroundColor: PRIMARY_COLOR,
              }}
              onMouseEnter={(e) => {
      ...
      >
        <div className="text-center space-y-3">
          ...
          <div className="flex items-center justify-center gap-1.5 text-[8px] text-emerald-600 font-bold bg-emerald-50 py-1.5 px-3 rounded-lg border border-emerald-200">
            ...
            />
            Auto-refresh QR: {qrCountdown} detik
          </div>

          <button
            onClick={() => setSelectedTicket(null)}
            className="w-full py-2.5 text-white rounded-xl text-[10px] font-bold transition cursor-pointer shadow-sm"
            style={{
              backgroundColor: PRIMARY_COLOR,
            }}
            onMouseEnter={(e) => {
        ...
        <div className="space-y-4 text-[10px]">
          ...
          <div className="space-y-2">
            <h4 className="font-bold text-neutral-800">
              Status Tracking Perjalanan
            </h4>

            <div
              className="space-y-2 border-l-2 pl-3 ml-1"
              style={{
                borderColor: `${PRIMARY_ACCENT}66`,
              }}
            >
              {selectedTicket?.trackingLogs?.map(
                (log, idx) => (
                  <div
                    key={idx}
                  ...
                  >
                    <div
                      className={`w-2.5 h-2.5 rounded-full absolute -left-4.25 top-0.5 ${
                        log.completed
                          ? ''
                          : 'bg-neutral-300'
                      }`}
          ...
          <button
            onClick={() => setSelectedTicket(null)}
            className="w-full py-2.5 text-white rounded-xl font-bold cursor-pointer transition"
            style={{
              backgroundColor: PRIMARY_COLOR,
            }}
            onMouseEnter={(e) => {
      ...
      >
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              type="submit"
              className="flex-1 py-2.5 text-white rounded-xl font-bold transition cursor-pointer shadow-sm"
              style={{
                backgroundColor: PRIMARY_COLOR,
              }}
              onMouseEnter={(e) => {
      ...
      >
        <div className="text-center space-y-4 text-[10px]">
          ...
          <p className="text-neutral-500">
            ...
            ? Kuota kursi/bagasi akan dikembalikan.
          </p>
