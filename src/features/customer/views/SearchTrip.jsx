...
const inferServiceType = (trip) =>
  Number(trip?.maxWeightCapacityKg) > 1 ? 'barang' : 'penumpang';
...
export default function SearchTrip() {
  ...
  useEffect(() => {
    ...
    const fetchPickupPoints = async () => {
      ...
      try {
        ...
        if (isMounted) {
          toast.error('Gagal memuat daftar pos resmi.', {
            title: 'Error',
          });
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTrips = async () => {
      ...
      try {
        ...
        if (origin) {
          params.originPointId = origin;
        }

        if (destination) {
          params.destinationPointId = destination;
        }
        ...
        if (isMounted) {
          ...
          const activeList = list.filter((t) => {
            ...
            const matchesServiceType =
              inferServiceType(t) === serviceType;
        ...
        if (isMounted) {
          toast.error('Gagal mengambil data trip dari server.', {
            title: 'Error',
          });
        }
      } finally {
  ...
  const isOverWeightCapacity =
    ...
    totalAccumulatedWeight >
      (selectedTrip?.remainingWeightCapacityKg || 0);

  const maxAllowedSeats =
    selectedTrip?.vehicle?.type === 'motor'
      ? 1
      : selectedTrip?.seatAvailable || 1;

  const isOverSeatCapacity =
    serviceType === 'penumpang' &&
    safeSeatCount > maxAllowedSeats;

  const isOverCapacity =
    isOverWeightCapacity || isOverSeatCapacity;

  const isPassengerPhoneValid =
    serviceType === 'penumpang'
      ? PHONE_REGEX.test(passengerPhone.trim())
      : true;

  const isReceiverPhoneValid =
    serviceType === 'barang'
      ? PHONE_REGEX.test(receiverPhone.trim())
      : true;

  const isPassengerValid =
    serviceType === 'penumpang'
      ? passengerName.trim() !== '' && isPassengerPhoneValid
      : true;

  const isBarangValid =
    serviceType === 'barang'
      ? receiverName.trim() !== '' && isReceiverPhoneValid
      : true;
  ...
  const handleSizeChange = (sizeCode) => {
    ...
    const selectedOpt = sizeOptions.find(
      (opt) => opt.code === sizeCode
    );
  ...
  const handlePinChange = (val, index) => {
    ...
    if (cleaned.length > 1) {
      ...
      document
        .getElementById(`pin-input-${nextIndex}`)
        ?.focus();
  ...
  const handleVerifyPinAndCheckout = async () => {
    ...
    try {
      ...
      const resOrder = await apiClient.post(
        '/orders',
        orderPayload
      );
      ...
      addTicket({
        ...
        title:
          serviceType === 'barang'
            ? `Nebeng Barang (${itemCategory})`
            : 'Nebeng Penumpang',

        from:
          selectedTrip.originPoint?.name ||
          'Pos Asal',

        to:
          selectedTrip.destinationPoint?.name ||
          'Pos Tujuan',

        mitra:
          selectedTrip.mitra?.name ||
          'Mitra',

        vehicle: selectedTrip.vehicle
          ? `${selectedTrip.vehicle.model} (${selectedTrip.vehicle.plateNumber})`
          : '-',
        ...
        trackingLogs: [
          {
            status:
              'Booking Dikonfirmasi & Dana Diamankan (Escrow)',
            location:
              selectedTrip.originPoint?.name,
            time: 'Baru saja',
            completed: true,
          ...
          {
            status: 'Arrived at Pos Destination',
            location:
              selectedTrip.destinationPoint?.name,
            time: '-',
            completed: false,
      ...
      toast.success(
        'Pembayaran sukses dan dana telah diamankan di Escrow.',
        {
          title: 'Sukses',
        }
      );
      ...
      toast.error(
        error.response?.data?.message ||
          'PIN salah atau gagal memproses pembayaran.',
        {
          title: 'Gagal',
        }
      );
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 min-h-screen font-['Inter']">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-neutral-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] sm:text-[11px] text-neutral-400 mt-0.5">
            Temukan perjalanan antar kota atau pengiriman barang
            dengan sistem keamanan Escrow terintegrasi.
          </p>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-9 pr-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl font-medium text-neutral-800 focus:outline-none focus:border-[#4FBF99] cursor-pointer appearance-none disabled:opacity-50"
              >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl font-medium text-neutral-800 focus:outline-none focus:border-[#4FBF99]"
              />
            </div>
          </div>
      ...
      <div className="space-y-3">
        <h2 className="text-[14px] font-bold text-neutral-800">
          Hasil Trip Tersedia ({trips.length})
        </h2>
        ...
        {isLoadingTrips ? (
          ...
          <div className="grid grid-cols-1 gap-3">
            {trips.map((trip) => (
              <div
                key={trip.id}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-neutral-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4"
              >
                <div className="space-y-1.5">

                  <div className="flex items-center gap-2">
                    <StatusBadge variant="purple">
                      {inferServiceType(trip) === 'penumpang'
                        ? 'Nebeng Penumpang'
                        : 'Nebeng Barang'}
                    </StatusBadge>
                  ...
                  <div className="flex flex-wrap gap-3 text-[9px] text-neutral-400 font-medium">
                    ...
                    <span>
                      ...
                      <strong style={{ color: PRIMARY_COLOR }}>
                        {inferServiceType(trip) ===
                        'penumpang'
                          ? `${trip.seatAvailable} Kursi`
                          : `${trip.remainingWeightCapacityKg} kg`}
                      </strong>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-2 w-full md:w-auto">
                  ...
                  <button
                    ...
                    className="w-full md:w-auto py-2.5 px-4 bg-[#4FBF99] hover:bg-[#429f80] text-white rounded-xl text-[10px] font-bold transition shadow-sm cursor-pointer"
                  >
                    Booking Sekarang
                  </button>
                </div>
      ...
      <BaseModal
        ...
        title={
          bookingStep === 'form'
            ? serviceType === 'penumpang'
              ? 'Formulir Nebeng Penumpang'
              : 'Formulir Nebeng Barang'
            : bookingStep === 'pin'
              ? 'Checkout & Escrow Payment'
              : 'Konfirmasi Selesai'
        }
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 bg-neutral-50 border rounded-xl font-bold text-neutral-800 focus:outline-none ${
                        ...
                        !isPassengerPhoneValid
                          ...
                          : 'border-neutral-200 focus:border-[#4FBF99]'
                      }`}
                    />
                  </div>
                </div>
              ...
              <>
                ...
                {isOverWeightCapacity && (
                  <p className="text-[8px] text-rose-600 font-bold">
                    ⚠️ Total berat ({totalAccumulatedWeight}{' '}
                    Kg) melebihi kapasitas sisa bagasi (
                    {selectedTrip?.remainingWeightCapacityKg}{' '}
                    Kg).
                  </p>
                )}
              </>
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl text-[10px] font-bold transition shadow-sm ${
                  !isFormValid
                    ...
                    : 'bg-[#4FBF99] hover:bg-[#429f80] text-white cursor-pointer'
                }`}
              >
          ...
          <div className="space-y-4 text-center py-2 text-[10px]">
            ...
            <div className="text-left space-y-1">
              ...
              >
                ...
                <option value="MANUAL_SIMULATION">
            ...
            <p className="text-[9px] text-neutral-400">
              Masukkan PIN transaksi rahasia Anda
              6-digit untuk konfirmasi Escrow.
            </p>
            ...
            <div className="flex justify-center gap-1.5">
              {pin.map((digit, index) => (
                <input
                  ...
                  className="w-8 h-9 text-center text-[12px] font-bold bg-neutral-50 border border-neutral-200 rounded-xl focus:outline-none focus:border-[#4FBF99] font-mono"
                />
              ))}
            </div>
        ...
        {bookingStep === 'success' && (
          <div className="text-center py-2 space-y-3 text-[10px]">
            ...
            <p className="text-neutral-500">
              Dana Anda telah berhasil ditahan oleh
              sistem Escrow sampai paket/penumpang tiba
              di Pos Tujuan.
            </p>
            ...
            <div className="flex gap-2 justify-center pt-2">
              ...
              <button
                ...
                className="py-2.5 px-3.5 bg-[#4FBF99] hover:bg-[#429f80] text-white rounded-xl font-bold shadow-sm cursor-pointer"
              >
                Lihat Tiket Saya
              </button>
            </div>
