...
export default function FinancialReportPage() {
  ...
  useEffect(() => {
    ...
    const loadFinancialData = async () => {
      try {
        ...
        const responseData =
          await regionalService.getPayments(currentRegionId);

        const formatted = responseData.map(item => ({
          id: String(
            item.id ||
            `TRX-${Math.floor(Math.random() * 9000 + 1000)}`
          ),
          ...
          status:
            ...
            item.status === 'success'
              ...
              : 'Pending',
          statusDetail:
            'Transaksi tervalidasi sistem database escrow.',
          date: item.createdAt
            ? new Date(item.createdAt).toLocaleString('id-ID')
            : 'Hari ini',
          paymentMethod:
            item.paymentMethod || 'QRIS / Non-Tunai'
        }));
  ...
  const totalCommission = filteredReports.reduce(
    (sum, item) =>
      sum + parseCurrency(item.commission),
    0
  );
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 min-h-screen font-['Inter'] print:p-0 print:bg-white">
                ...
                <tr>
                  <td colSpan="6">
                    <EmptyState
                      ...
                      description="Belum ada data transaksi di database."
                    />
                  </td>
                </tr>
              )}
      ...
      >
        <div className="space-y-3 text-[10px]">
          <div>
            <strong>ID Trip:</strong>{' '}
            {selectedStatusDetail?.tripId}
          </div>

          <div>
            <strong>Metode Pembayaran:</strong>{' '}
            {selectedStatusDetail?.paymentMethod}
          </div>

          <div>
            <strong>Catatan Settlement:</strong>{' '}
            {selectedStatusDetail?.statusDetail}
          </div>
