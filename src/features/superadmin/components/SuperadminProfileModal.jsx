...
export default function SuperadminProfileModal({ isOpen, onClose, session, role, superadminProfile, onSettingsClick }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-[22px] max-w-sm w-full p-6 shadow-2xl space-y-5 text-gray-900">
        <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
          <h3 className="text-[16px] font-semibold text-neutral-800">Profil Saya</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-3">
          {avatarUrl ? (
            ...
            <div className="w-14 h-14 rounded-full bg-brand-900 text-white flex items-center justify-center font-bold text-[18px] shrink-0">
              {initials}
            </div>
          )}
          <div className="min-w-0">
            <p className="text-[14px] font-semibold text-neutral-800 truncate">{displayName}</p>
            <p className="text-[11px] text-neutral-400 truncate">{displayRole}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 text-[12px] font-semibold rounded-full transition cursor-pointer"
          >
            Tutup
          </button>
          <button
            onClick={handleGoToSettings}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-brand-900 hover:bg-brand-950 text-white text-[12px] font-semibold rounded-full transition cursor-pointer"
          >
